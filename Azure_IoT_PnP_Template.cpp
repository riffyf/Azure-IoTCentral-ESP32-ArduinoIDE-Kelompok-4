...
/* --- Defines --- */
#define az_failed(result) ((result) != AZ_OK)
...
#define TELEMETRY_PROP_NAME_PPMGas "None"
#define TELEMETRY_PROP_NAME_obstacle "None"
#define TELEMETRY_PROP_NAME_Distance "Distance"
#define TELEMETRY_PROP_NAME_Api "None"
...
const int MQ2_DO_PIN = 5; // Digital output MQ-2
const int MQ2_AO_PIN = 4; // Analog output MQ-2
const int trigPin = 19;   // Trig pin untuk ultrasonik
const int echoPin = 21;   // Echo pin untuk ultrasonik
...
// Fungsi untuk membaca nilai MQ-2 (ppm)
static float get_mq2_value() {
  return analogRead(MQ2_AO_PIN); // Membaca nilai analog (ppm)
}

// Fungsi untuk membaca jarak menggunakan HC-SR04
...
static void control_relays(float distance, float ppm) {
    ...
    size_t* payload_buffer_length)
{
  ...
  float PPMGas, Distance;

  // Membaca data dari sensor
  PPMGas = get_mq2_value();
  get_ultrasonic_values(&Distance);

  // Kontrol relay berdasarkan pembacaan sensor
  control_relays(Distance, PPMGas);
  ...
  // Tambahkan data MQ-2
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("PPMGas"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding PPMGas property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, PPMGas, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding PPMGas property value to telemetry payload.");
  ...
  // Tambahkan status relay 1
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("VibrationMotor"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding Relay1Status property name to telemetry payload.");
  rc = az_json_writer_append_bool(&jw, relay1_status);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding Relay1Status property value to telemetry payload.");

  // Tambahkan status relay 2
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("Buzzer"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding Relay2Status property name to telemetry payload.");
  rc = az_json_writer_append_bool(&jw, relay2_status);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding Relay2Status property value to telemetry payload.");
